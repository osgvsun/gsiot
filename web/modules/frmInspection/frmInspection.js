App.code.debug.frmInspection.code=function(name,app){
    var self = this;Form.call(this, name);this.app = app; this.parent = app.parent;this.Timer = null;this.taskTimer=null;
    this.load = function () {
    ...
    this.Form_Show=function(){
        ...
        // else {$$("#showpic").src("http://"+Document.Paths.server+":"+this.app.cfg.server.webservice.uvccamera.port+this.app.cfg.server.webservice.uvccamera.streamer)}
        else {$$("#showpic").src("/mjpgstreamer/picamera")}
        this.Form_Resize(Document.Width(),Document.Height());
        if (this.taskTimer==null) {this.taskTimer=setInterval(this.task,1000);};
    };
    ...
    this.task=function(){
		...
		if(self.app.policy.end<now){
			if(self.taskTimer!=null){window.clearInterval(self.taskTimer);self.taskTimer=null;};
            if(self.Timer!=null){self.cmdStopMoive_Click($$("#off_videotape").hWnd)};
            if(self.app.cfg.station.webcam!="")self.cmdStopNetworkMoive_Click($$("#off_networkvideotape"));
    ...
    this.start_countdown_videoTimer=function(a) {
        ...
        this.Timer=window.setInterval(function(){
            ...
            let showTime = `${h}:${m}:${s}`
            if(a.html()=="")a.html("<b>● REC "+showTime+"</b>");
            else a.Clear();
            d = Number(d)
            h = Number(h)
            m = Number(m)
    ...
    this.cmdLive_Click=function(e){
        ...
        url="http://"+Document.Paths.server+":"+this.app.cfg.server.webservice.uvccamera.port+this.app.cfg.server.webservice.uvccamera.streamer
        url="/mjpgstreamer/picamera"
        img.src(url)
    ...
    this.cmdStartMoive_Click=function(a){
        ...
        this.app.wsc.SendMessage({
            ...
            "courseid":self.app.policy.courseid,
            "experimentid":self.app.policy.experimentid,
            "OperatorUserName":self.app.policy.username
        });
    };
