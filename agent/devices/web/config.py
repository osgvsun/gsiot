...
def setConfig(route,filename,request):
    if request.request_data!={}:
        file=gsJsonFile(app.rootpath+"/app/etc/"+filename)
        file.data=edict(request.request_data)
        file.Savefile()
        app.logservice("info",request.request_data)
        return {"result":True}
    ret={"filename":app.rootpath+"/app/etc/"+filename,"isexists":os.path.isfile(app.rootpath+"/etc/conf/"+filename)}
    if ret["isexists"]:ret=json.load(open(app.rootpath+"/app/etc/"+filename))
    return ret
